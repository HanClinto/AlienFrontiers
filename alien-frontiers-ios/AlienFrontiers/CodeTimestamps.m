...
double SecondsFromTimeInterval(uint64_t timeInterval) {
    return NanosecondsFromTimeInterval(timeInterval) / 1000000000.0;
}

uint64_t TimeIntervalFromNanoseconds(uint64_t timeInterval) {
    static struct mach_timebase_info timebase_info;
    OneTimeCall(mach_timebase_info(&timebase_info));
    timeInterval /= timebase_info.numer;
    timeInterval *= timebase_info.denom;
...
uint64_t TimeIntervalFromSeconds(double timeInterval) {
    return (TimeIntervalFromNanoseconds((uint64_t)(timeInterval * 1000000000)));
}
    ...
    @synchronized(chunkData) {
      ...
      if (thisThreadHadChunks) {
        NSLog(@"++ Chunk = %@, avg time = %llu nsec", chunkName,
              totalNanoSecsThisThread / ((numRunsThisThread != 0) ? numRunsThisThread : 1));
      }
